...
Assume the database is SQLite and there is a table called 'sales_data.db' with the following columns:
...
1. "Show total sales and quantity per city" means:
   SELECT City, SUM(Sales) AS Total_Sales, SUM(Quantity) AS Total_Quantity FROM sales_data.db GROUP BY City

2. "Which city had the highest sales in 2024" means:
   SELECT City, SUM(Sales) AS Total_Sales FROM sales_data.db WHERE sale_date BETWEEN '2024-01-01' AND '2024-12-31' GROUP BY City ORDER BY Total_Sales DESC LIMIT 1

3. "Get monthly sales for Product 2 in 2025" means:
   SELECT strftime('%Y-%m', sale_date) AS Month, SUM(Sales) AS Total_Sales FROM sales_data.db WHERE Product_Name = 'Product 2' AND sale_date BETWEEN '2025-01-01' AND '2025-12-31' GROUP BY Month ORDER BY Month

4. "Show top 3 cities by total quantity sold" means:
   SELECT City, SUM(Quantity) AS Total_Quantity FROM sales_data.db GROUP BY City ORDER BY Total_Quantity DESC LIMIT 3

5. "List product names with their total sales" means:
   SELECT Product_Name, SUM(Sales) FROM sales_data.db GROUP BY Product_Name

6. "Find total quantity sold for each channel in the last 6 months" means:
   SELECT Channel, SUM(Quantity) FROM sales_data.db WHERE sale_date >= date('now', '-6 months') GROUP BY Channel

7. "What is the average sales per transaction for Product 2" means:
   SELECT AVG(Sales) FROM sales_data.db WHERE Product_Name = 'Product 2'

8. "Rank cities based on total sales" means:
   SELECT City, SUM(Sales) AS Total_Sales, RANK() OVER (ORDER BY SUM(Sales) DESC) AS Rank FROM sales_data.db GROUP BY City

9. "Get sales in City1 for Channel 1 in October 2024" means:
   SELECT * FROM sales_data.db WHERE City = 'City1' AND Channel = 'Channel 1' AND sale_date BETWEEN '2024-10-01' AND '2024-10-31'

10. "Compare sales in January and February 2025" means:
    SELECT strftime('%Y-%m', sale_date) AS Month, SUM(Sales) FROM sales_data.db WHERE sale_date BETWEEN '2025-01-01' AND '2025-02-28' GROUP BY Month

11. "What are the monthly sales across platform1 since Jan 2025?" means:
    SELECT strftime('%Y-%m', sale_date) AS Month, SUM(Sales) AS Total_Sales FROM sales_data.db WHERE Channel = 'Channel 1' AND sale_date >= '2025-01-01' GROUP BY Month ORDER BY Month;

12. "What is the share of units sold across various platforms since Jan 2025?" means:
    SELECT Channel, SUM(Quantity) AS Total_Quantity, (SUM(Quantity) / (SELECT SUM(Quantity) FROM sales_data.db WHERE sale_date >= '2025-01-01')) * 100 AS Share_Percent FROM sales_data.db WHERE sale_date >= '2025-01-01' GROUP BY Channel;

13. "Can you tell me the top 5 days with the highest daily units sold?" means:
    SELECT sale_date, SUM(Quantity) AS Total_Quantity FROM sales_data.db GROUP BY sale_date ORDER BY Total_Quantity DESC LIMIT 5;
"""
...
# ✅ Function to execute SQL query on SQLite3
    ...
    if question.strip() == "":
        ...
        with st.spinner("Generating SQL and fetching data..."):
            ...
            if sql_query:
                ...
                # List tables in the SQLite database to check if 'sales_data.db' exists
                tables = list_tables(db_path)
                
                # Execute SQL query and show results if 'sales_data.db' exists
                if ('sales_data.db',) in tables:
                    ...
                    st.error("Table 'sales_data.db' does not exist in the database.")
